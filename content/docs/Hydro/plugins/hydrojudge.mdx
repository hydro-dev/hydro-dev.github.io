...
<Callout type="warn">
Windows 并没有真正的沙盒支持，请不要在生产环境下使用！！！
</Callout>
...
在 [criyle/go-judge](https://github.com/criyle/go-judge/releases/) 处下载对应系统架构的 go-judge，并保持运行（监听 5050端口）。

##### 下载评测机组件

创建 `hydro-judge` 文件夹

在 [hydro-dev/Hydro](https://github.com/hydro-dev/Hydro/tree/master/packages) 里下载 [common](https://github.com/hydro-dev/Hydro/tree/master/packages/common) 、[hydrojudge](https://github.com/hydro-dev/Hydro/tree/master/packages/hydrojudge)、[utils](https://github.com/hydro-dev/Hydro/tree/master/packages/utils) 三个包，将它们放入`hydro-judge` 文件夹。

从 [MikeMirzayanov/testlib](https://github.com/MikeMirzayanov/testlib/releases) 处下载 `testlib.h` 并放入 `hydro-judge/hydrojudge/vendor/testlib` 

##### <Callout>

有时需要修改 `hydro-judge/hydrojudge` 内的  `package.json` ，将自身添加到依赖中。
</Callout>

再在`hydro-judge` 文件夹目录下创建一个 `package.json` 内容如下：
...
在 `hydro-judge` 文件夹下运行：

```
node hydrojudge/bin/hydrojudge
```
