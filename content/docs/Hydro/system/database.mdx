...
- `--dbOnly` 跳过文件备份，仅备份数据库。
- `--withAddons` 同时备份安装的插件
- `--withLogs` 同时备份系统日志

## 增量备份

自 Hydro 5.0.0-beta.6 版本起，支持基于 restic 进行增量备份。  
首次使用请先执行 `restic init` 初始化一个存储库，之后使用 `hydrooj backup -r 存储库地址 -p 存储库密码` 进行备份。
