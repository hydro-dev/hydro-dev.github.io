...
参照本文 [想要创建一个属于自己的OJ吗？](https://hydro.ac/discuss/6172ceeed850d38c79ae18f9) 无服务器快速体验系统功能。  
...
(() => {
    ...
    (r.clearRect(0, 0, l, u),
      s.forEach(function (e, n) {
        for (
          e.x += e.xa,
            e.y += e.ya,
            e.xa *= e.x > l || e.x < 0 ? -1 : 1,
            e.ya *= e.y > u || e.y < 0 ? -1 : 1,
            r.fillRect(e.x - 0.5, e.y - 0.5, 1, 1),
            o = n + 1;
          o < d.length;
          o++
        )
          null !== (t = d[o]).x &&
            ...
            (m = i * i + a * a) < t.max &&
              (t === y &&
                m >= t.max / 2 &&
                ((e.x -= 0.03 * i), (e.y -= 0.03 * a)),
              (m = (t.max - m) / t.max),
              r.beginPath(),
              (r.lineWidth = m / 2),
              (r.strokeStyle = "rgba(" + x.c + "," + (0.2 + m) + ")"),
              r.moveTo(e.x, e.y),
              r.lineTo(t.x, t.y),
              r.stroke()));
      }),
  ...
  var l,
    ...
    x =
      ((t = (o = document.getElementsByTagName("script")).length),
      (o = o[t - 1]),
      {
        l: t,
        z: e(o, "zIndex", -1),
  ...
  ((i.id = a),
    (i.style.cssText =
      "position:fixed;top:0;left:0;z-index:" + x.z + ";opacity:" + x.o),
    document.getElementsByClassName("main")[0].appendChild(i),
    n(),
    (window.onresize = n),
  ...
  for (var s = [], h = 0; x.n > h; h++) {
    var f = m() * l,
      ...
      p = 2 * m() - 1,
      v = 2 * m() - 1;
    s.push({ x: f, y: g, xa: p, ya: v, max: 6e3 });
  }
  ((d = s.concat([y])),
    setTimeout(function () {
      c();
...
查看 /dev/shm 的大小：`df -h /dev/shm`

调整 /dev/shm 的大小：`sudo mount -o remount,size=1024M /dev/shm`
...
反向代理时请确保设置了正确的 Host Header。[详细说明](./install/proxy)
...
`cd 
`yarn global dir` && rm -rf node_modules yarn.lock && yarn install` 后再次尝试。
