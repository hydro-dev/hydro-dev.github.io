    ...
    export default {
        ...
        watch: {
            content() {
                ...
                if (this.static) return this.startStatic();
            }
        },
        async mounted() {
            if (this.static) return this.startStatic();
            while (1) await this.start();
        },
        methods: {
            getLines() {
            ...
            async start() {
                ...
                for (const line of this.lines) {
                    ...
                    if (line.type === "select") {
                        ...
                        line.content[index].text =
                            ">" + line.content[index].text.slice(1);
        ...
        render(createElement) {
            ...
            return createElement(
                "panel-view",
                {
                    ...
                    style: {
                        height:
                            (this.lines.length * 1.4 + (mini ? 2 : 3.4)) * 16 + "px"
                    },
                    props: {
                        title: this.title,
                        controls: !mini
                ...
                this.lines.map(
                    ({ type, active, content, shown, message }, index) => {
