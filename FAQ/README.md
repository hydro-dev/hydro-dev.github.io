...
创建插件后在插件 `frontend/animation.page.ts` 中写入以下内容：
...
Judge 会将临时文件存储于 /dev/shm 中，包含选手的输出、编译的可执行文件等等。这些文件会在评测完成后被自动删除。  
如果您设置的并行评测数量较高，请注意调整 /dev/shm 的大小限制，我们通常建议至少预留 512MB 的空间。
